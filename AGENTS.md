...
0. **Ronda 43 (v2 — LEER PRIMERO):** el flujo estrella ahora es "Tarjetas QR de Docentes" (protocolo `CLASE:v2:{teacherId}:{subjectSlug}:{exp}:{sig}`, sin grado/día/bloque). Probar: Horarios → QR de Clase → tarjeta de un docente → "Ver tarjeta QR" (o "Imprimir todas") → escanear en el Escáner DENTRO de un bloque CLASE → chip "Clase activa: {materia} · {docente} · {bloque}" → escanear carnés de CUALQUIER curso → registros con la asignatura de la tarjeta + grado del carné + contexto "QR de Clase (firmado)". El Portal Docente tiene "Mis Tarjetas QR" (autogestión) y "Activar mi asignatura" (1-toque v2). La v1 (por cátedra) sigue viva como sub-opción colapsable y conserva su gate de grado. Suite local: `TZ=America/Bogota bun scripts/verify_ronda43.ts` (40/40).
