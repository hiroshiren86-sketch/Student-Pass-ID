...
**Decir (esta es la parte que conviene decir con exactitud):** *"El representante es **una alternativa** para cuando el profesor no quiere o no puede pasar lista: el salón se apoya en su representante. Pero no es un administrador: él aporta **hechos** de asistencia, nunca toca la lista oficial de estudiantes, los horarios ni las credenciales — la autoridad sobre el catálogo la tiene solo Rectoría. Y hay dos cosas que quiero dejar claras de frente: la primera, que su celular no guarda la llave con la que se firman los carnés, así que la tarjeta de clase se valida contra la nube del colegio en el momento; la segunda, que **la sincronización automática de sus escaneos todavía no está implementada**: se dejó así por compatibilidad, para no romper la sincronización que ya funciona. Hoy **los portales autorizados para el autosincronizado son el del Docente y el de Rectoría**: su escáner reenvía lo pendiente y su push publica el estado del dispositivo."*

**Aclaración técnica honesta (para la ronda de preguntas, no para narrar):** el escaneo del representante **se guarda al instante** en su dispositivo y **entra a una cola de envío**; esa cola reenvía los hechos al servidor por la ruta de asistencia (idempotente, un hecho por escaneo, nunca el catálogo). **El dato no se pierde**: viaja cuando ese dispositivo se sincroniza desde una sesión de Docente/Rectoría (el Escáner reenvía lo pendiente al recuperar la conexión), o cuando ese equipo publica su estado con un push. Lo que hoy **no** ocurre es que el portal del estudiante publique solo: en la prueba en producción del 15/09/2026 (informe R70) ese fue **el único punto parcial de 30 comprobaciones**. Y ojo con un detalle: **no** es cierto que personalizar la foto o cargar el horario en CSV obliguen a publicar — la Ronda 64 lo bloqueó a propósito (son personalización del dispositivo). Todo esto quedó verificado con código en ejecución: **12 comprobaciones en `tests/evidence/r70_alcance_autosync.txt`**. Si preguntan "¿ya está arreglado?": responda *"está diagnosticado, con corrección propuesta y verificable"*, y no prometa el ciclo automático de 5 minutos para el portal del estudiante.
...
- **Si el representante escanea desde su propio celular, ¿el registro llega a la nube?** Sí, y llega como **hecho de asistencia** (nunca como catálogo) — pero **no automáticamente desde su portal**: la sincronización automática de sus escaneos **no está implementada** (se dejó así por compatibilidad, para no romper lo que ya funciona). El registro se guarda al instante en el teléfono y queda en la cola de envío; se publica cuando ese dispositivo sincroniza con una sesión de **Docente o Rectoría** (los portales autorizados para el autosincronizado) o cuando ese equipo hace un push. En la prueba en producción del 15/09/2026 ese fue el único punto parcial de 30 comprobaciones. Detalle verificado: personalizar la foto o cargar el horario en CSV **no** fuerza la publicación (Ronda 64). Análisis y propuesta: `ANALISIS_R70_OUTBOX_REPRESENTANTE.md`; verificación: `tests/evidence/r70_alcance_autosync.txt` (12 comprobaciones).
...
- **Alcance del autosincronizado (12 comprobaciones, `tests/evidence/r70_alcance_autosync.txt`):** hoy los portales autorizados para el autosincronizado son los de **Docente y Rectoría**; el portal del estudiante/representante guarda y encola, pero **la sincronización automática de sus escaneos no está implementada** (decisión de compatibilidad para no romper lo que ya funciona). Tampoco es cierto que "personalizar foto" o "cargar horario CSV" fuercen la publicación: la Ronda 64 lo bloqueó a propósito.
...
- **El único punto parcial (fila 11 del informe):** el auto-registro quedó guardado en el teléfono, pero **su publicación automática desde el portal del estudiante no está implementada** — se dejó así a propósito, por **compatibilidad**: los portales autorizados hoy para el autosincronizado son el de **Docente** y el de **Rectoría** (su Escáner reenvía los hechos pendientes y su push publica el estado del dispositivo). No es pérdida de datos: el hecho queda guardado y en cola duradera hasta que ese dispositivo sincronice desde una sesión de Docente/Rectoría. Verificado con código en ejecución (`tests/unit/r70_alcance_autosync.ts` » **12 OK**, evidencia `tests/evidence/r70_alcance_autosync.txt`): en sesión de estudiante, ni el escaneo, ni la foto, ni el horario CSV sellan "hay cambios sin subir". El análisis con líneas de código y la corrección propuesta (3 cambios aditivos, sin abrir permisos nuevos) están en `ANALISIS_R70_OUTBOX_REPRESENTANTE.md`.
