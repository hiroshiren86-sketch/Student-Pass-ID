...
export const TeacherClassroomView: React.FC<TeacherClassroomViewProps> = ({ 
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-6 rounded-3xl bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800/50 shadow-xl space-y-4">
        {/* Ronda 19 — QR de Clase: contexto activo del dispositivo (expira al fin del bloque) */}
        <ActiveClassBanner />
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button
            type="button"
            onClick={() => {
              const res = AttendanceStorageService.activateClassDirect(selectedGrade, activeSlot.id);
            ...
            className="px-3.5 py-2 rounded-xl bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-800 text-indigo-700 dark:text-indigo-300 text-[11px] font-bold hover:bg-indigo-100 dark:hover:bg-indigo-900 transition-all flex items-center gap-1.5 shrink-0"
            title="Vincula los escaneos de este curso a la materia del bloque activo, aunque el reloj no coincida"
            aria-label="Activar clase en este dispositivo"
          >
